...
services:
  green-agent:
    ...
    command: ["python", "-m", "src.green_agent.server", "--host", "0.0.0.0", "--port", "{green_port}", "--card-url", "http://green-agent:{green_port}"]
    env_file:
      - .env
...
{participant_services}
  agentbeats-client:
    ...
    volumes:
      ...
      - ./output:/app/output
    entrypoint: ["/bin/sh", "-c"]
    command: ["sleep 15 && uv run src/agentbeats/client_cli.py scenario.toml output/results.json"]
...
PARTICIPANT_TEMPLATE = """  {name}:
    ...
    platform: linux/amd64
    container_name: {name}
    command: ["python", "-m", "src.purple_agents.baseline_agent", "--agent-id", "{name}", "--host", "0.0.0.0", "--port", "{participant_port}", "--public-url", "http://{name}:{participant_port}"]
    env_file:
      - .env
    environment:{env}
    networks:
